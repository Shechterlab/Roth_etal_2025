---
title: "PROSeq Gene Expression Analysis and Comparison: GSK591 Timecourse in A549"
output:
  html_document:
    toc: true
  pdf_document:
...
params:
  input_filename: "C:/Users/david/OneDrive/Bioinformatics/PRMTi/proseq/peppro/comparisons.txt"
  genes_to_highlight: NULL
  tpm_data_file: "C:/Users/david/OneDrive/Bioinformatics/PRMTi/proseq/peppro/20250608_GSK591_PROseq_TPM.txt"
  ...
  num_clusters: 4 #kmeans clusters for zscore plot 
  num_genes_to_plot_zscore: 500
  ...
  output_dir: "C:/Users/david/OneDrive/Bioinformatics/PRMTi/proseq/peppro/Output/"
---
...
read_data <- function(file_path) {
  ...
  # Map to Entrez ID
  df <- df %>%
    mutate(entrez_id = NA_character_)

  if ("ENSEMBL" %in% df$id_type) {
    ens_ids <- unique(df$gene_id[df$id_type == "ENSEMBL"])
    ens_map <- AnnotationDbi::select(orgdb, keys = ens_ids, keytype = "ENSEMBL", columns = "ENTREZID")
    df <- left_join(df, ens_map, by = c("gene_id" = "ENSEMBL")) %>%
      mutate(entrez_id = coalesce(entrez_id.y, entrez_id)) %>%
      select(-entrez_id.y)
  }

  if ("SYMBOL" %in% df$id_type) {
    sym_ids <- unique(df$gene_id[df$id_type == "SYMBOL"])
    sym_map <- AnnotationDbi::select(orgdb, keys = sym_ids, keytype = "SYMBOL", columns = "ENTREZID")
    df <- left_join(df, sym_map, by = c("gene_id" = "SYMBOL")) %>%
      mutate(entrez_id = coalesce(entrez_id.y, entrez_id)) %>%
      select(-entrez_id.y)
  }

  df$id_type <- NULL  # optional: clean up
  return(df)
}
...
```{r VolcanoPlots, warning=FALSE, message=FALSE}


# Prepare biotype filtering
mart <- biomaRt::useEnsembl("ensembl", dataset = "hsapiens_gene_ensembl")
biotype_table <- biomaRt::getBM(
  attributes = c("hgnc_symbol", "gene_biotype"),
  filters = "hgnc_symbol",
  values = unique(unlist(lapply(data_list, \(x) x$GeneSymbol))),
  mart = mart
)
biotype_table <- biotype_table[!duplicated(biotype_table$hgnc_symbol), ]
allowed_symbols <- biotype_table$hgnc_symbol[biotype_table$gene_biotype %in% c("protein_coding", "lncRNA")]

# Volcano plot function
create_volcano_plot <- function(data, title, max_genes_to_highlight = params$max_genes_to_highlight) {
  data <- data %>%
    filter(GeneSymbol %in% allowed_symbols) %>%
...
```{r TPM_heatmap, eval=TRUE, warning=FALSE, message=FALSE}

# Load TPM data
tpm_data <- readr::read_tsv(params$tpm_data_file)
stopifnot("GeneSymbol" %in% colnames(tpm_data))

# Drop Control1 columns
tpm_data <- tpm_data[, !grepl("Control1", colnames(tpm_data)) | colnames(tpm_data) == "GeneSymbol"]

# Extract TPM matrix and gene symbols
gene_symbols <- tpm_data$GeneSymbol
tpm_matrix <- as.matrix(tpm_data[, setdiff(colnames(tpm_data), "GeneSymbol")])
...
# Order samples
ordered_columns <- c(
  "Control2_REP1", "Control2_REP2",
  "15m_REP1", "15m_REP2",
  "3hr_REP1", "3hr_REP2",
  "2days_REP1", "2days_REP2",
  "4days_REP1", "4days_REP2",
  "7days_REP1", "7days_REP2"
)
...
# Get gene biotypes from Ensembl using biomaRt
mart <- biomaRt::useEnsembl("ensembl", dataset = "hsapiens_gene_ensembl")
biotype_table <- biomaRt::getBM(
  attributes = c("hgnc_symbol", "gene_biotype"),
  filters = "hgnc_symbol",
...
biotype_table <- biotype_table[!duplicated(biotype_table$hgnc_symbol), ]
allowed_biotypes <- c("protein_coding", "lncRNA")
tpm_matrix <- tpm_matrix[rownames(tpm_matrix) %in% biotype_table$hgnc_symbol[biotype_table$gene_biotype %in% allowed_biotypes], ]
...
# Save heatmap as PDF
pdf(file.path(plot_output_dir, "TPM_Zscore_Heatmap_PROseq_GSK591.pdf"), width = 10, height = 7)
draw(heatmap_plot)
dev.off()
...
# Reuse the TPM data from earlier (already loaded as `tpm_data`)
# Ensure proper sample filtering
tpm_filtered <- tpm_data[, !grepl("Control1", colnames(tpm_data)) | colnames(tpm_data) == "GeneSymbol"]

# Extract TPM matrix and rownames
tpm_matrix <- as.matrix(tpm_filtered[, setdiff(colnames(tpm_filtered), "GeneSymbol")])
rownames(tpm_matrix) <- tpm_filtered$GeneSymbol
tpm_matrix <- apply(tpm_matrix, 2, as.numeric)
rownames(tpm_matrix) <- tpm_filtered$GeneSymbol

#prevent cache lock issues
options(biomaRt.cache = FALSE)  # Disable local cache to prevent lock issues

# Filter to protein-coding and lncRNA genes only using biomaRt
mart <- biomaRt::useEnsembl("ensembl", dataset = "hsapiens_gene_ensembl")
biotype_table <- biomaRt::getBM(
  attributes = c("hgnc_symbol", "gene_biotype"),
  filters = "hgnc_symbol",
...
biotype_table <- biotype_table[!duplicated(biotype_table$hgnc_symbol), ]
allowed_symbols <- biotype_table$hgnc_symbol[biotype_table$gene_biotype %in% c("protein_coding", "lncRNA")]
tpm_matrix <- tpm_matrix[rownames(tpm_matrix) %in% allowed_symbols, ]
...
sample_info <- data.frame(
  ...
  Condition = gsub("_REP[12]", "", colnames(log_tpm_filtered))
)
...
ylgnbu_colors <- brewer.pal(n = 9, name = "YlGnBu")[3:(4 + n_colors - 1)]
names(ylgnbu_colors) <- unique(pca_df$Condition)

# Compute centroid for labeling ellipses
...
p <- ggplot(pca_df, aes(x = PC1, y = PC2, color = Condition)) +
  ...
  # Use ggrepel to label centroids
  geom_text_repel( data = centroids, aes(label = Condition), size = 6, color = "black", fontface = "plain",  
  nudge_x = 15,
  nudge_y = 15,
  point.padding = 0.8,
  box.padding = 0.5,
...
# Save and display
ggsave(file.path(plot_output_dir, "PCA_TPM_PROseq_GSK591.pdf"), plot = p, width = 6, height = 5)
print(p)
```
